...
module Model = struct
  ...
  let get_focus_point (field,dir) =
    let dir_int = match dir with Buy -> 0 | Sell -> 1 in
  ...
  let focus_map ~collapsed =
    if collapsed then collapsed_focus_map else full_focus_map
  ...
  let example =
    ...
    (fun () ->
       let name = String.init 4 ~f:(fun _ ->
         Char.to_int 'A' + Random.State.int rng 26
         |> Char.of_int_exn)
       in
...
module Action = struct
  ...
  let apply t focus_point (m:Model.t) =
    ...
    | Bump (bdir) ->
      begin
        let amt = match bdir with Incr -> 0.01 | Decr -> -0.01 in
        ...
        | Some focus_point ->
          ...
          | Some pos ->
            let basic =
              Model.Basic.fset m.basic pos
                (Model.Basic.get m.basic pos +. amt)
            in
            { m with basic }
      end
    | Toggle_collapse ->
...
let header ~collapsed ~set_inner_focus focus =
  ...
  let dstring = function Buy -> "/b" | Sell -> "/s" in
  let live_header =
    let s = "live" in
    [ th [] [Node.text (s ^ dstring Buy)]
    ; th [] [Node.text (s ^ dstring Sell)] ]
  in
  let bd ff dir =
    th
...
let view (m:Model.t Incr.t) (entry_id:Entry_id.t) ~visible ~focus ~focus_me ~set_inner_focus =
  ...
  let table =
    let focused = match focus with Focused _ -> true | Unfocused -> false in
    Node.table
