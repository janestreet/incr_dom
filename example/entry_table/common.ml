...
module Dirpair = struct
  ...
  let get t = function Buy -> t.buy | Sell -> t.sell
...
type bump_dir = Incr | Decr [@@deriving sexp]
type focus_dir = Next | Prev [@@deriving sexp]

let move_map_focus map focus dir =
  match focus with
  | None ->
    let find = match dir with
      | Prev -> Map.max_elt
      | Next -> Map.min_elt
    in
    Option.map ~f:fst (find map)
  | Some index ->
    let condition = match dir with
      | Prev -> `Less_than
      | Next -> `Greater_than
    in
